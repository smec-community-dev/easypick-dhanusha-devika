...
def seller_dashboard(request):
    ...
    sales_data = [0, 0, 0, 13998, 0, 0, 0, 0, 0, 0, 0, 0]

    max_sales = max(sales_data) if max(sales_data) > 0 else 1

    chart_data = []

    for value in sales_data:
        percent = (value / max_sales) * 100
        chart_data.append(percent)

    orders_pending = Order.objects.filter(
    items__product__seller__isnull=False,
    items__product__seller=seller_profile,
...
#         discount_value = request.POST.get("discount_percent")  # rename to match form
#         discount_type = "percentage"  # assuming you want % discounts
#         start_date = request.POST.get("start_date")
#         end_date = request.POST.get("end_date")

#         variant = ProductVariant.objects.get(id=variant_id)

#         # Convert discount_value to float/int
#         discount_value = float(discount_value)

#         # Create Discount
#         Discount.objects.create(
#             product=variant.product,     # link discount to product
